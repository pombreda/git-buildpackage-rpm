...
class GbpConfig(object):
    ...
    default_config_files = ['/etc/git-buildpackage/gbp.conf',
                            '~/.gbp.conf',
                            '%(top_dir)s/.gbp.conf',
    ...
    def get_config_files(cls, no_local=False):
        ...
        files = envvar.split(':') if envvar else cls.default_config_files
...
class GbpConfArgParser(object):
    """Wrapper around argument parser and argument groups. Common
    functionality for GBP-specific argument parser and argument groups.
    That is, config file options and argument prefixing."""
    ...
    class _GbpArgParser(ArgumentParser):
        ...
        def __init__(self, **kwargs):
            ...
            if prog and not (prog.startswith('git-') or prog.startswith('gbp-')):
